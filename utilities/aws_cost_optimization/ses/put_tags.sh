...
do
	ses_arn=`cat "$FILE" | jq -r '.ResourceArn'`
	ses_name=`echo "$ses_arn" | awk -F/ '{print $NF}'`
	echo Updating tags for $ses_name from "$FILE"
	aws sesv2 tag-resource --resource-arn $ses_arn --cli-input-json file://"$FILE"
done