...
job_config_file="/opt/scripts/config/rds_execute_sql.json"

repo_base_url=https://svn.franklinmadisonds.com/svn/insurance/database
repo_path=$2
...
if [[ "${account}" == "Choose One" || "${application}" == "Choose One" ]];
then
...
printf "${PURPLE}Application : ${GREEN}$application${NC}\n"
...
# Check account and application combination
if [[ "${account}" == "pci" ]]; then
	if [[ "${application}" == "hermes2.0" || "${application}" == "hermes3.0" ]]; then
		printf "${RED}Wrong account and application combination.\n"
		exit 1
	fi
...
if [[ "${account}" == "dev" ]]; then
	db_host=rds-postgres-priv-dev1.crsdewuox0qj.us-east-1.rds.amazonaws.com
	db_pass=$admin_pass_dev_lasso
elif [[ "${account}" == "qa" ]]; then
	db_host=rd-postgres-priv-stage1.catwxniuzvrl.us-east-1.rds.amazonaws.com
	db_pass=$admin_pass_stage_lasso
elif [[ "${account}" == "stage" ]]; then
	if [[ "${application}" == "hermes2.0" || "${application}" == "hermes3.0" ]]; then
		db_host=rds-pg-fmg-serverless-stg.catwxniuzvrl.us-east-1.rds.amazonaws.com
		db_pass=$admin_pass_stage_hermes2
	elif [[ "${application}" == "lasso" ]]; then
		db_host=rd-postgres-priv-stage1.catwxniuzvrl.us-east-1.rds.amazonaws.com
		db_pass=$admin_pass_stage_lasso
	fi
fi
...
do
	...
	then
		...
		psql -h $db_host -p 5432 -d $db_name -U postgres -f $processedFile > query_output.txt 2>&1
