resource "aws_batch_compute_environment" "hms_py_batch_proc_ce_8xlarge" {
	count = var.hermes_pycharm_batch_processor_8xlarge_bool ? 1: 0
	compute_environment_name = var.ce_name_8xlarge
	...
	compute_resources {
		...
		instance_type = var.ce_cr_instance_type_8xlarge
		max_vcpus = var.ce_cr_max_vcpus_8xlarge
...
resource "aws_batch_job_queue" "hms_py_batch_proc_jq_8xlarge" {
	count = var.hermes_pycharm_batch_processor_8xlarge_bool ? 1: 0
	name					= var.jq_name_8xlarge
	state					= "ENABLED"
	priority				= 1
	compute_environments	= [aws_batch_compute_environment.hms_py_batch_proc_ce_8xlarge[0].arn]
}

resource "aws_batch_job_definition" "hms_py_batch_8xlarge_proc_jd" {
	count = var.hermes_pycharm_batch_processor_8xlarge_bool ? 1: 0
	name = var.jd_name_8xlarge
	type = "container"
	...
	timeout {
		attempt_duration_seconds = var.jd_timeout_8xlarge
	}
	container_properties = jsonencode({
		...
		{
			type  = "VCPU"
			value = var.jd_container_vcpu_8xlarge
		},
		{
			type  = "MEMORY"
			value = var.jd_container_memory_8xlarge
		}
		]
...
resource "aws_cloudwatch_event_rule" "hms_py_batch_8xlarge_proc_rule" {
	count = var.hermes_pycharm_batch_processor_8xlarge_bool ? 1: 0
	name = var.eb_rule_name_8xlarge
	tags = var.eb_rule_tags
	state = "DISABLED"

	event_pattern = jsonencode(var.eb_rule_event_pattern_8xlarge)
...
resource "aws_cloudwatch_event_target" "hms_py_batch_8xlarge_proc_target" {
	count = var.hermes_pycharm_batch_processor_8xlarge_bool ? 1: 0
	rule = aws_cloudwatch_event_rule.hms_py_batch_8xlarge_proc_rule[0].name
	arn = aws_batch_job_queue.hms_py_batch_proc_jq_8xlarge[0].arn
	role_arn = var.event_target_role_arn

	batch_target {
		job_definition	= aws_batch_job_definition.hms_py_batch_8xlarge_proc_jd[0].arn
		job_name		= var.event_target_job_name_8xlarge
	}
