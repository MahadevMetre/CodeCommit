...
resource "aws_cloudwatch_log_group" "log-group5" {
	name = var.cloudwatch_log_group_name5
	retention_in_days = var.cloudwatch_log_group_retention5
	tags = var.cloudwatch_log_group_tags5
}
...
resource "aws_ecs_task_definition" "my_ecs_task_definition5" {
	family = "${var.task_definition_family5}"

	container_definitions = jsonencode([{
		name = var.container_definition_name5
		image = var.container_definition_image5
		essential = true

		logConfiguration = {
			logDriver = "awslogs"
			options = {
				awslogs-group = aws_cloudwatch_log_group.log-group5.id
				awslogs-region = "us-east-1"
				awslogs-stream-prefix = "ecs"
			}
		}
		...
		portMappings = [
		{
			containerPort = var.cd_portmapping_containerport5
			hostPort = var.cd_portmapping_hostport5
			protocol = var.cd_portmapping_protocol5
		}
		]

		environment = var.cd_env_vars5

		mountPoints = var.cd_mount_points5
	...
	requires_compatibilities	= ["EC2", "FARGATE"]
	network_mode				= "awsvpc"
	cpu							= var.task_definition_cpu5
	memory						= var.task_definition_memory5
	execution_role_arn			= length(aws_iam_role.ecsTaskExecutionRole) > 0 ? aws_iam_role.ecsTaskExecutionRole[0].arn : var.td_execution_role_arn5
	task_role_arn				= length(aws_iam_role.ecsTaskExecutionRole) > 0 ? aws_iam_role.ecsTaskExecutionRole[0].arn : var.td_task_role_arn5
...
data "aws_ecs_task_definition" "my_ecs_task_definition_data5" {
	task_definition = aws_ecs_task_definition.my_ecs_task_definition5.family
}
...
resource "aws_ecs_service" "my_ecs_service5" {
	name                 = var.service_name5
	cluster              = aws_ecs_cluster.my_cluster.id
	task_definition      = "${aws_ecs_task_definition.my_ecs_task_definition5.family}:${max(aws_ecs_task_definition.my_ecs_task_definition5.revision, data.aws_ecs_task_definition.my_ecs_task_definition_data5.revision)}"
	launch_type          = "FARGATE"
	scheduling_strategy  = "REPLICA"
	desired_count        = 0
	force_new_deployment = true
	...
	network_configuration {
		subnets				= var.service_subnets5
		assign_public_ip	= false
		security_groups		= var.service_sg5
	}

	load_balancer {
		target_group_arn	= var.service_tg_arn5
		container_name		= var.container_definition_name5
		container_port		= var.cd_portmapping_containerport5
	}
	# The following block is added to ignore the service count changes which are made manually.
	# Comment out in case we need to update the desired_count via terraform
	lifecycle {
