...
resource "aws_iam_policy" "batch-integration_ses_access_policy" {
  name = "batch-integration-ses-access-policy"
  path = "/"
  policy = <<EOF
{
    "Version": "2012-10-17",
...
resource "aws_iam_policy" "batch-integration-s3policy" {
  name        = "batch-integration-s3policy"
  path        = "/"
  description = "Policy that allows access to specific S3 operations on buckets with names starting with customercommunications-"
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Resource": [
                "arn:aws:s3:::customercommunications-*"
            ],
            "Sid": "VisualEditor0"
        }
    ]
...
resource "aws_iam_role_policy_attachment" "batch-integration-s3policy_attachment" {
  role = aws_iam_role.ecsTaskExecutionRole[0].name
  policy_arn = aws_iam_policy.batch-integration-s3policy.arn
}

resource "aws_iam_role_policy_attachment" "batch-integration_ses_access_policy_attachment" {
  role = aws_iam_role.ecsTaskExecutionRole[0].name
  policy_arn = aws_iam_policy.batch-integration_ses_access_policy.arn
}
