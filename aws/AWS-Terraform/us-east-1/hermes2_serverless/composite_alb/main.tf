...
module "loadbalancer" {
	...
	# security_groups					= ["${module.alb_security_group.sgOut.id}"]
...
module "target_group2" {
	...
	success_codes					= var.tg2_success_codes
...
module "target_group3" {
	...
	success_codes					= var.tg3_success_codes
...
module "lb_rule" {
	...
	priority				= var.lbr_priority
	action_type				= var.lbr_action_type
	target_group_arn		= module.target_group2.tgOut.arn
	path_pattern			= var.lbr_path_pattern
	tags					= var.lbr_tags
}
