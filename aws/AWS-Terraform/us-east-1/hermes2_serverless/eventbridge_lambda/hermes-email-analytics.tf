resource "aws_scheduler_schedule" "hms_email_analytics_schedule" {
	count = var.hermes_email_analytics_bool ? 1: 0
	name = var.schedule_name
	group_name = var.schedule_group_name
	description = "Scheduler to invoke Hermes Email Analytics Lambda"
