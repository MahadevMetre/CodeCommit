resource "aws_scheduler_schedule" "hms_kibana_count_schedule" {
	count = var.hermes_kibana_count_api_bool ? 1: 0
	name = var.schedule_name
	group_name = var.schedule_group_name
	description = "Scheduler to invoke Hermes Kibana Count Lambda"
