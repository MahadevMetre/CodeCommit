resource "aws_batch_compute_environment" "hms_addr_standardization_ce" {
	count = var.hermes_addr_standardization_bool ? 1: 0
...
resource "aws_batch_job_queue" "hms_addr_standardization_jq" {
	count = var.hermes_addr_standardization_bool ? 1: 0
	name					= var.jq_name
	state					= "ENABLED"
	priority				= 1
	compute_environments	= [aws_batch_compute_environment.hms_addr_standardization_ce[0].arn]
}

resource "aws_batch_job_definition" "hms_addr_standardization_jd" {
	count = var.hermes_addr_standardization_bool ? 1: 0
	name = var.jd_name
	type = "container"
...
resource "aws_cloudwatch_event_rule" "hms_addr_standardization_rule" {
	count = var.hermes_addr_standardization_bool ? 1: 0
...
resource "aws_cloudwatch_event_target" "hms_addr_standardization_target" {
	count = var.hermes_addr_standardization_bool ? 1: 0
	rule = aws_cloudwatch_event_rule.hms_addr_standardization_rule[0].name
	arn = aws_batch_job_queue.hms_addr_standardization_jq[0].arn
	role_arn = var.event_target_role_arn

	batch_target {
		job_definition	= aws_batch_job_definition.hms_addr_standardization_jd[0].arn
