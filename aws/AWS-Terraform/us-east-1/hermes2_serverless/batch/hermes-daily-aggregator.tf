resource "aws_batch_compute_environment" "hms_daily_agg_ce" {
	count = var.hermes_daily_aggregator_bool ? 1: 0
...
resource "aws_batch_job_queue" "hms_daily_agg_jq" {
	count = var.hermes_daily_aggregator_bool ? 1: 0
	name					= var.jq_name
	state					= "ENABLED"
	priority				= 1
	compute_environments	= [aws_batch_compute_environment.hms_daily_agg_ce[0].arn]
}

resource "aws_batch_job_definition" "hms_daily_agg_jd" {
	count = var.hermes_daily_aggregator_bool ? 1: 0
	name = var.jd_name
	type = "container"
...
resource "aws_scheduler_schedule" "hermes_daily_aggregator" {
	count = var.hermes_daily_aggregator_bool ? 1: 0
	name = var.schedule_name
	group_name = var.schedule_group_name
	description = "Scheduler to invoke Hermes Daily Aggregator Job"
	...
	target {
		...
		input = jsonencode({
			JobDefinition	= aws_batch_job_definition.hms_daily_agg_jd[0].arn
			JobName			= var.schedule_job_name
			JobQueue		= aws_batch_job_queue.hms_daily_agg_jq[0].arn
		})
